...
- **편집 UI**
  ...
  - textarea(실제 수정 입력)
- **“수정된 곳 표시” 효과(줄 단위 강조)**
  - 목적: 사용자가 긴 OCR 결과를 편집할 때, **어느 줄이 바뀌었는지 즉시 확인**할 수 있게 함
  - 방식(프론트 `monitor/page.tsx`):
    - `editedText.split('\n')`로 현재 편집 텍스트를 줄 배열로 분해
    - 같은 인덱스의 원본 줄을 `selectedRecord.extracted_text.split('\n')[idx]`로 가져와 비교
    - `line !== originalLine` 이면 “수정됨”으로 판단하여 해당 줄에 강조 스타일 적용
  - UI 표현:
    - 변경된 줄: `bg-red-50` + `border-l-4 border-red-500` + 글자색을 빨간 계열로 표시
    - 변경되지 않은 줄: 일반 텍스트 스타일
  - 안내 문구:
    - `editedText !== selectedRecord.extracted_text` 인 경우, “수정됨: 위 비교 뷰에서 빨간색으로 표시된 줄이 원본과 다른 내용” 메시지를 노출
  - 한계/주의:
    - 이 비교는 “진짜 diff 알고리즘”이 아니라 **줄 인덱스 기반 비교**입니다.
    - 중간에 줄 삽입/삭제가 많으면 이후 줄들이 연쇄적으로 “수정됨”으로 표시될 수 있습니다(현재 UX는 단순/빠른 확인용).
- **저장 호출**
